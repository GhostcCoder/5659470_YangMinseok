...
void insert_edge(GraphType* g, int start, int end, int w) {
    g->edges[g->n].start = start; // 간선의 시작 정점 설정
    g->edges[g->n].end = end; // 간선의 끝 정점 설정
    g->edges[g->n].weight = w; // 간선의 가중치 설정
    g->weight[start][end] = w; // 양방향
    g->weight[end][start] = w; //삽입
    g->n++; // 간선의 수 증가
}

// 그래프 생성 함수 - 계속 쭉 써온 그래프...
